...
def compute_mfcc_vectors(file_path, dim_type=13, n_fft=None, strict=True):
    """Tính MFCC với phân khung tự code: frame=30ms, shift=10ms.
    Trả về ma trận (num_frames x dim) theo dim_type: 13/26/39.
    """
    y, _ = librosa.load(file_path, sr=FS)
    y_steady = extract_stable_segment(y, FS, strict_mode=strict)

    # Tham số khung
    frame_len = int(0.03 * FS)   # 30ms -> 480 mẫu
    hop_len   = int(0.01 * FS)   # 10ms -> 160 mẫu

    # Tự phân khung tín hiệu
    def frame_signal(sig, frame_len, hop_len):
        if len(sig) < frame_len:
            sig = np.pad(sig, (0, frame_len - len(sig)))
        num_frames = 1 + int((len(sig) - frame_len) / hop_len) if len(sig) >= frame_len else 1
        frames = np.zeros((num_frames, frame_len), dtype=np.float32)
        for i in range(num_frames):
            start = i * hop_len
            end = start + frame_len
            if end > len(sig):
                # pad cuối nếu thiếu mẫu
                segment = np.pad(sig[start:], (0, end - len(sig)))
            else:
                segment = sig[start:end]
            # cửa sổ Hamming để giảm rò rỉ phổ
            frames[i, :] = segment * np.hamming(frame_len)
        return frames

    frames = frame_signal(y_steady, frame_len, hop_len)

    # Tính MFCC cho từng khung dựa trên mel-spectrogram của khung
    mfcc_list = []
    for fr in frames:
        # Mel-spectrogram cho 1 khung (một vector tần số)
        S = librosa.feature.melspectrogram(y=fr, sr=FS, n_fft=frame_len, hop_length=None, center=False, power=2.0)
        # Log-mel
        log_mel = librosa.power_to_db(S, ref=np.max)
        # MFCC từ log-mel (kết quả dạng (n_mfcc, 1))
        mf = librosa.feature.mfcc(S=log_mel, n_mfcc=13)
        mfcc_list.append(mf[:, 0])

    mfcc = np.stack(mfcc_list, axis=1)  # (13, num_frames)
